...
//  2022-05-23: OpenGL: Reworking 2021-12-15 "Using buffer orphaning" so it only
//  happens on Intel GPU, seems to cause problems otherwise. (#4468, #4825,
//  #4832, #5127). 2022-05-13: OpenGL: Fix state corruption on OpenGL ES 2.0 due
//  to not preserving GL_ELEMENT_ARRAY_BUFFER_BINDING and vertex attribute
//  states. 2021-12-15: OpenGL: Using buffer orphaning + glBufferSubData(),
//  seems to fix leaks with multi-viewports with some Intel HD drivers.
//  2021-08-23: OpenGL: Fixed ES 3.0 shader ("#version 300 es") use normal
//  precision floats to avoid wobbly rendering at HD resolutions. 2021-08-19:
//  OpenGL: Embed and use our own minimal GL loader
//  (imgui_impl_opengl3_loader.h), removing requirement and support for
//  third-party loader. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX
//  access changed to bd->XXXX). 2021-06-25: OpenGL: Use OES_vertex_array
...
bool ImGui_ImplOpenGL3_Init(const char *glsl_version) {
  ...
  if (const char *vendor = (const char *)glGetString(GL_VENDOR))
    if (strncmp(vendor, "Intel", 5) == 0)
      ...
      // '%s'\nGL_RENDERER = '%s'\n", major, minor, (const
      // char*)glGetString(GL_VENDOR), (const char*)glGetString(GL_RENDERER)); //
      // [DEBUG]
#else
...
static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData *draw_data,
  ...
  glVertexAttribPointer(bd->AttribLocationVtxPos, 2, GL_FLOAT, GL_FALSE,
                        sizeof(ImDrawVert),
                        (GLvoid *)IM_OFFSETOF(ImDrawVert, pos));
  glVertexAttribPointer(bd->AttribLocationVtxUV, 2, GL_FLOAT, GL_FALSE,
                        sizeof(ImDrawVert),
                        (GLvoid *)IM_OFFSETOF(ImDrawVert, uv));
  glVertexAttribPointer(bd->AttribLocationVtxColor, 4, GL_UNSIGNED_BYTE,
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data) {
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    // - On Intel windows drivers we got reports that regular glBufferData() led
    // to accumulating leaks when using multi-viewports, so we started using
    // orphaning + glBufferSubData(). (See
    // https://github.com/ocornut/imgui/issues/4468)
    // - On NVIDIA drivers we got reports that using orphaning +
    // glBufferSubData() led to glitches when using multi-viewports.
    // - OpenGL drivers are in a very sorry state in 2022, for now we are
    // switching code path based on vendors.
    ...
    if (bd->UseBufferSubData) {
      if (bd->VertexBufferSize < vtx_buffer_size) {
        ...
        glBufferData(GL_ARRAY_BUFFER, bd->VertexBufferSize, NULL,
                     GL_STREAM_DRAW);
      }
      if (bd->IndexBufferSize < idx_buffer_size) {
        bd->IndexBufferSize = idx_buffer_size;
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, bd->IndexBufferSize, NULL,
                     GL_STREAM_DRAW);
      }
